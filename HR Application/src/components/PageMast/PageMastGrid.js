...
import AddPageMast from "./AddPageMast";
import EditPageMast from "./EditPageMast";
...
var intervalId = "";
class PageMastGrid extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedIndex: 0,
      ...
      pagemasterstate: [],
      seletedstateid: [],
      editpagemast: "editpagemast",
      addpagemast: "addpagemast",
      add: "",
      selected: null,
      columns: [
        {
  ...
  handleBtnClick = () => {
    this.props.history.push("/editPageMast/" + this.state.selected);
  };
 ...
 edit = () => {
   debugger;
    ...
    setTimeout(() => {
      this.props.userInfo({
        ...
        epmd: this.state.pagemasterstate[this.state.selectedIndex]["Id"],
      });
    }, 100);
  ...
  if (result.value) {
    ...
    axios
    .post(
      `${axios.baseURL}/HRAPI/PostVisbileByID_TableName?TableName=PageMast&ID=${name}`,
      this.state )
    .then((response) => {
  ...
  exportToExcle = () => {
   const fileType =
    ...
    let fileName = "Pagemast/" + new Date().getTime();
  ...
  printTable = () => {
    ...
    table += "<table classname='table table-bordered'}><tr><th>Id</th><th>Page Name</th><th>Display Name</th><th>Status</th><th>Created By</th><th>Created Date</th><th>ModifyBy</th><th>Modified Date</th></tr>";
    for (let i = 0; i < this.state.filterdata.length; i++) {
      let pagemasterstate = this.state.filterdata[i];
      table +=
        "<tr><td>" +
        pagemasterstate.Id +
        "</td><td>" +
        pagemasterstate.PageName +
        "</td><td>" +
        pagemasterstate.DisplayName +
        "</td><td>" +
        pagemasterstate.DisplayYN +
        "</td><td>" +
        pagemasterstate.CreatedBy +
        "</td><td>" +
        pagemasterstate.CreatedDate +
        "</td><td>" +
        pagemasterstate.ModifyBy +
        "</td><td>" +
        pagemasterstate.ModifiedDate +
        "</td></tr>";
    }
    ...
    printWin.print();
    // window.print(this.state.pagemasterstate);
  };
  ...
  render() {
    ...
    const options = {
      ...
      sizePerPageList: [
        ...
        {
          text: "All",
          value: this.state.pagemasterstate.length,
        },
      ],
      sizePerPage: 10,
      pageStartIndex: 1,
    ...
    return (
      <>
        ...
        <div className="modal-header">
                <h4>Edit Page Master</h4>
                
              </div>
       {this.state.pagemasterstate[this.state.selectedIndex] && (
            <EditPageMast
            EditPageMastId={
              this.state.selected
              }
        ...
        <div className="modal-header">
          ...
          {this.state.employee[this.state.selected] && (
            <EditDepartmentcls
              departmentId={
                this.state.employee[this.state.selected]["Id"]
              }
              closeModal={this.closeModal2}
            />
          )}
        ...
        <div >
          <div style={{ marginTop: 10 }} className="table-responsive">
            <div style={{ marginLeft: 5 }}>
              <Gridcontrolbtn
                addpagemast={this.state.addpagemast}
            ...
            <BootstrapTable
              ...
              hover
              keyField="Id"
              data={this.state.pagemasterstate}
