...
const addTotalcol = (data, hiddencol, byColumn, seprator) => {
    ...
    for (let key of keys) {
        if (key === keys[0]) {
            totalRow += seprator +  '<td style="font-weight:bold;">Total</td>';
        } else if (key !== keys[0]) {       // (key === byColumn)
            var p=0;
            for (var j = 0; j < byColumn.length; j++) {
                if (key == byColumn[j]) {
                    
                    totalRow += '<td style="font-weight:bold;">'+ total[j] + '</td>'; 
                    p=1;
                }
            }
            if(p===0){
...
export function GroupcsvExportWithFiveColumn(allColumns, allData, columnhidden, groupColName,ReportTitle,TopHeader) {  //addtotal,addAvg,addCount,
    ...
    var strheader = '<table border=1><tr style="font-weight:bold;font-size:22;text-align:center;background-color:cyan;"><td colspan='+collength+'>'+ReportTitle+ '</td></tr>';
                            ...
                            for (var index in array1[p]) {
                                ...
                                if (checkExist === -1) {
                                    if (index != 'tableData') {
                                        ...
                                        { 
                                            if(temp=='Total'){
                                                line += '<td style="font-weight:bold;font-size:14">'+temp+'</td>';   
                                                continue; 
                                            }
    ...
    var today=getExcelFileName();
    var exportFileName=today+".xls";
    var csv = TopHeader + strheader + line;
    var excel_blob = new Blob([csv], {type:'data:application/vnd.ms-excel;base64'}); 
    var link = document.createElement("a");
    var url = URL.createObjectURL(excel_blob);
...
export function GroupcsvExportWithFourColumn(allColumns, allData, columnhidden, groupColName,ReportTitle,TopHeader) {  //addtotal,addAvg,addCount,
    var jsonHeader = JSON.stringify(allColumns);
    var header = typeof jsonObject != 'object' ? JSON.parse(jsonHeader) : jsonHeader;
...
export function GroupcsvExportWithThreeColumn(allColumns, allData, columnhidden, groupColName,ReportTitle,TopHeader) {  //addtotal,addAvg,addCount,
    var jsonHeader = JSON.stringify(allColumns);
    var header = typeof jsonObject != 'object' ? JSON.parse(jsonHeader) : jsonHeader;
...
export function GroupcsvExportWithTwoColumn(allColumns, allData, columnhidden, groupColName,ReportTitle,TopHeader) {  //addtotal,addAvg,addCount,
    var jsonHeader = JSON.stringify(allColumns);
    var header = typeof jsonObject != 'object' ? JSON.parse(jsonHeader) : jsonHeader;
...
export function GroupcsvExportWithOneColumn(allColumns, allData, columnhidden, groupColName,ReportTitle,TopHeader) {  //addtotal,addAvg,addCount,
    debugger;
    var jsonHeader = JSON.stringify(allColumns);
    var header = typeof jsonObject != 'object' ? JSON.parse(jsonHeader) : jsonHeader;
...
export function GroupcsvExportWithZeroColumn(allColumns, allData, columnhidden,groupColName,addtotal,addAvg,addCount,ReportTitle,TopHeader) {
    var jsonHeader = JSON.stringify(allColumns);
    var header = typeof jsonObject != 'object' ? JSON.parse(jsonHeader) : jsonHeader;
    var strheader = '<table border=1><tr style="font-weight:bold;font-size:22;text-align:center;background-color:cyan;"><td colspan='+allColumns.length+'>'+ReportTitle+' </td></tr><tr style="background-color:pink;font-size:16">';
    var hiddencol = [];
    for (var ch = 0; ch < columnhidden.length; ch++) {
        hiddencol[ch] = columnhidden[ch]['field']
    }
    ...
    var today=getExcelFileName();
    var exportFileName=today+".xls";
    var csv =TopHeader + strheader + line1
    var excel_blob = new Blob([csv], {type:'data:application/vnd.ms-excel;base64'}); 
    var link = document.createElement("a");
    var url = URL.createObjectURL(excel_blob);
              link.setAttribute("href", url);
